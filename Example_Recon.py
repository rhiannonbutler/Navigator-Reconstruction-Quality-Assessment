...
from coils import calculate_csm_inati_iter
...
# THIS WAS CHANGED: now we use the walsh method of coil combination and keep the sos combination of bin dimension

mag_coil_first = np.moveaxis(mag, -1, 0)
# shape: (nc, nx, ny, n, bins, neco)
#use walsh method to combine coil dim
combined = np.zeros((nx, ny, nbins, neco), dtype=mag.dtype)
print(mag_coil_first.shape)

for b in range(nbins):
    for e in range(neco):
        coil_imgs = mag_coil_first[:, :, :, b, e]   # (nc, nx, ny) — now truly 2D+coil
        csm, combined_img = calculate_csm_inati_iter(coil_imgs, smoothing=5, niter=5)
        combined[:, :, b, e] = combined_img

combined = sos(combined.transpose((0, 1, 3, 2)))  # combine across bins
# output shape: (nx, ny, neco, nc)
y_vis = np.arange(96,224)

# plot all magnitude of all echoes
_, ax = plt.subplots(1, neco, figsize=(12,12*(2/neco))) 
for i in range(neco):
    ax[i].imshow(np.rot90(combined[:,y_vis,i]), vmin=0, vmax=np.max(combined)*.8, cmap='gray')
    ax[i].set_title(f'Recon Echo {i}')


# save results as nifti
img = nib.Nifti1Image(combined, np.eye(4))
